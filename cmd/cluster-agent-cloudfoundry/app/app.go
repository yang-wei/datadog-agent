...
once per cluster.`,
	...
	startCmd = &cobra.Command{
		...
		Long:  `Runs Datadog Cluster agent for Cloud Foundryin the foreground`,
		RunE:  start,
	}
...
func initializeBBSCache() error {
	var cps []config.ConfigurationProviders
	err := config.Datadog.UnmarshalKey("config_providers", &cps)
	if err != nil {
		return fmt.Errorf("Failed reading config_providers: %v", err)
	}

	var cfProvider *config.ConfigurationProviders = nil
	for _, cp := range cps {
		if cp.Name == names.CloudFoundryBBS {
			cfProvider = &cp
		}
	}
	if cfProvider == nil {
		return fmt.Errorf("%s provider not configured", names.CloudFoundryBBS)
	}

	pollInterval := providers.GetPollInterval(*cfProvider)
	// NOTE: we can't use GetPollInterval in ConfigureGlobalBBSCache, as that causes import cycle
	bc, err := cloudfoundry.ConfigureGlobalBBSCache(*cfProvider, pollInterval, true)
	if err != nil {
